...
object BigramHMMTagger extends App {
  ...
  var hyperParameters: Tuple2[Double, Double] = (1.0, 0.0)
  ...
  def writePredictedLabelsToDisk(testingData: List[List[String]], predictedLabels: List[List[String]]) = {
    val writer = new PrintWriter("bigramHmm-" + new File(System.currentTimeMillis() + ".json"))
    ...
    testingData.zipWithIndex.foreach({ case (line: List[String], linePosition: Int) => {
      ...
      line.zipWithIndex.foreach({ case (word: String, wordPosition: Int) => {
        ...
        posUnigramCount.keySet.filter((pos: String) => pos != StopToken).foreach((posTag: String) => {
          lookupTable(wordPosition)(posTag) = mutable.Map[String, Any]()
          lookupTable(wordPosition)(posTag)(wordStr) = word
          lookupTable(wordPosition)(posTag)(tagStr) = posTag
          lookupTable(wordPosition)(posTag)(emissionProbabilityStr) = Math.log((1.0 * dictionaryPosCount.getOrElse(word, mutable.Map()).getOrElse(posTag, 0)) / posUnigramCount.get(posTag).get) / Math.log(2.0)
          lookupTable(wordPosition)(posTag)(transitionProbabilityStr) = Double.MinValue
          if (wordPosition == 0) {
            lookupTable(wordPosition)(posTag)(transitionProbabilityStr) = bigramProbability(StartToken, posTag)
          }
          else {
            posUnigramCount.keySet.filter((pos: String) => pos != StopToken).foreach((previousPosTag: String) => {
              val transitionProbability = bigramProbability(previousPosTag, posTag) + lookupTable(wordPosition - 1)(previousPosTag)(finalProbabilityStr).asInstanceOf[Double]
              if (transitionProbability > lookupTable(wordPosition)(posTag)(transitionProbabilityStr).asInstanceOf[Double]) {
                lookupTable(wordPosition)(posTag)(transitionProbabilityStr) = transitionProbability
                lookupTable(wordPosition)(posTag)(backPointerStr) = lookupTable(wordPosition - 1)(previousPosTag)
              }
            })
          }

          if (wordPosition == line.size - 1) {
            lookupTable(wordPosition)(posTag)(transitionProbabilityStr) = lookupTable(wordPosition)(posTag)(transitionProbabilityStr).asInstanceOf[Double] + bigramProbability(posTag, StopToken)
          }

          lookupTable(wordPosition)(posTag)(finalProbabilityStr) = lookupTable(wordPosition)(posTag)(emissionProbabilityStr).asInstanceOf[Double] + lookupTable(wordPosition)(posTag)(transitionProbabilityStr).asInstanceOf[Double]
        })
      }
      })
  ...
  def backTrack(lookupTable: mutable.Map[Int, mutable.Map[String, mutable.Map[String, Any]]], winningCell: mutable.Map[String, Any]): List[String] = {
    ...
    wordsWithPos += winningCell(tagStr).asInstanceOf[String]
    var currentTag = winningCell(tagStr).asInstanceOf[String]
    for (index <- (1 to lookupTable.size - 1).reverse) {
      val backPointer = lookupTable(index)(currentTag)(backPointerStr).asInstanceOf[mutable.Map[String, Any]]
      wordsWithPos += backPointer(tagStr).asInstanceOf[String]
      currentTag = backPointer(tagStr).asInstanceOf[String]
    }
    return wordsWithPos.reverse.toList
  }

  def bigramProbability(previousPosTag: String, posTag: String): Double = {
    val bigramBase: Double = if (previousPosTag == StartToken) posUnigramCount(StopToken) else posUnigramCount(previousPosTag)
    val bigramScore: Double = (hyperParameters._1 * posBigramCount.getOrElse(previousPosTag, mutable.Map()).getOrElse(posTag, 0)) / bigramBase
    val unigramScore: Double = (hyperParameters._2 * posUnigramCount(posTag)) / trainingSetWordCount

    return Math.log(bigramScore + unigramScore) / Math.log(2.0)
  }
